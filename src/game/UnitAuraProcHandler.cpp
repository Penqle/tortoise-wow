            ...
            switch (dummySpell->Id)
            {
                ...
                case 51990:
                {
                    ...
                    if (!procSpell || !(procSpell->GetSpellSchoolMask() & SPELL_SCHOOL_MASK_ARCANE))
                        return SPELL_AURA_PROC_FAILED; // only arcane spells duplicate
...
SpellAuraProcResult Unit::HandleProcTriggerSpellAuraProc(Unit* pVictim, uint32 damage, int32 /*originalAmount*/, Aura* triggeredByAura, SpellEntry const *procSpell, uint32 procFlags, uint32 procEx, uint32 cooldown)
{
    ...
    switch (auraSpellInfo->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_PALADIN:
        {
            ...
            {
                // Requires Righteous Fury (25780) on the caster.
                if (!HasAura(25780))
                    return SPELL_AURA_PROC_FAILED;
            ...
            else if (auraSpellInfo->SpellIconID == 241)
            {
                ...
                basepoints[0] = originalSpell->manaCost * triggerAmount / 100;
